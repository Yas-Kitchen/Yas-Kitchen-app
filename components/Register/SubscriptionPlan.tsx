...
const SubscriptionPlan = () => {
  const { monthlyPlan, setActiveStep } = useGlobalContext();
    ...
    <View className="bg-base_color/10 rounded-2xl p-5">
      ...
      <View className="flex-col">
        <View className="flex-row w-1/2 gap-5 mt-5">
          ...
          <Text className="mx-8">{monthlyPlan} Plan</Text>
        </View>
        <View className="flex-row w-1/2 gap-5 mt-5">
          <Text className="text-base_color text-[12px]">Monthly Price :</Text>
          <Text className="mx-7 text-primary">{monthlyPlan === 'Diet' ? '100' : monthlyPlan === 'Regular' ? '85' : '75'} /month</Text>
        </View>
